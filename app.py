...
def load_and_prepare_data():
    ...
    if 'Month' not in meta_raw.columns or 'Amount spent (USD)' not in meta_raw.columns:
        ...
        st.stop()
    meta_raw['Month Start'] = pd.to_datetime(meta_raw['Month'].str.split(' - ').str[0], errors='coerce')
    ...
    if 'Month' not in google_raw.columns or 'Cost' not in google_raw.columns:
        ...
        st.stop()
    google_raw['Month Start'] = pd.to_datetime(google_raw['Month'], format='%B %Y', errors='coerce')
...
if shop_files and meta_file and google_file:
    ...
    if st.button("Run Scenario Forecast"):
        ...
        # Formatting
        scenario['Historical Sales']      = scenario['Historical Sales'].map("${:,.2f}".format)
        for c in ['Scenario Meta Spend','Scenario Google Spend','Estimated Sales','Dollar Increase']:
